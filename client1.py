...
def handle_communication(client_socket, screen):
    ...
    while not game_over:
        if turn == 1:  # Espera jogada do adversário
            data = client_socket.recv(1024).decode()
            if data:
                row, col = map(int, data.split(","))
                drop_piece(board, row, col, 2)  # Atualiza o tabuleiro com a jogada recebida
                lock.acquire()
                turn = 0
                lock.release()
...
def main():
    ...
    pygame.display.set_caption("Connect4 - Client 1")

    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server_socket.bind((HOST, PORT))
    server_socket.listen(1)
    print("Aguardando conexão do Client 2...")
